...
static NSString * const ZSSEditorHTML = @"RE.getHtml();";
static NSString * const ZSSEditorText = @"RE.getText();";
static NSString * const ZSSEditorContent = @"document.activeElement.id=='editor-text-area'";
static NSInteger const ZSSEditorMaxTitleLength = 30;
static NSString * const ZSSEditorTitlePlaceHolder = @"请填写标题，最多30字";

@class ZSSBarButtonItem;

/**
 *  The viewController used with ZSSRichTextEditor
 *  base js files include jQuery JSBeatutifier and editor，but it can't implement 100% feature of chinese editor
 *  indent、outdent、p h switch etc；demo is in WTEditorViewController_03
 *  ZHP has altered with wangEditor to implement perfect chinese char editing feature, demo is in WTEditorViewController_02
 *  add custom layout with top title TextView, webview is updated height when editing
 */
@interface ZSSRichTextEditor : UIViewController <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, HRColorPickerViewControllerDelegate, UITextViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, ZSSFontsViewControllerDelegate, UIScrollViewDelegate>
 ...
 */
@property (nonatomic, strong, readonly) UIView *toolbarHolder;
 ...
 */
@property (nonatomic, strong) ZSSWebView *editorView;

/*
 *  Keyboard Observe func
 */
- (void)keyboardWillShowOrHide:(NSNotification *)notification;
/*
 *  frame for update
 */
- (void)updateWebContainerFrame:(CGRect)frame;
...
/**
 *  Utilities func Returns the plain text from the Rich Text Editor
 *
 */
